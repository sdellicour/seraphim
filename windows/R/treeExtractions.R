treeExtractions <-
function (localTreesDirectory, allTrees, burnIn, randomSampling, 
    ...
    nberOfCores = 1) 
{
    ...
    for (i in 1:length(sampledNewTreeListMinusBurnIn)) {
        ...
        if (MCCtree == FALSE) {
            ...
            cat("Exctracting information from sampled tree n\302\260", 
                treeID, "\n", sep = "")
        }
        else {
            treeID = "MCC_tree"
        ...
        if (l > 0) {
            ...
            file = as.character(paste(localTreesDirectory, "/TreeExtractions_", 
                i, ".csv", sep = ""))
