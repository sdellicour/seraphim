...
function (file, tree.names = NULL) 
{
    strip_annotations = function(text) {
        ...
        repeat {
            ...
            text = sub(pattern, paste("[", end, "]", sep = ""), 
                text)
            end = end + 1
        }
    ...
    split_tree_traits = function(text) {
        text = regmatches(text, regexpr(pattern = "\\[.*?\\]", 
            text))
    ...
    annotated_tree_build = function(tp) {
        ...
        add.terminal = function() {
            ...
            if (length(annotations) > 0) {
                permute[[j]] <<- annotations[[as.numeric(X[2])]]
            }
            k <<- k + 1L
            tip <<- tip + 1L
            j <<- j + 1L
        ...
        go.down = function() {
            ...
            if (length(annotations) > 0) {
                permute[[l]] <<- annotations[[as.numeric(X[2])]]
            }
            k <<- k + 1L
        ...
        if (m != -1) {
            root.annotation.number = as.numeric(gsub("\\[(\\d+)\\];", 
                "\\1", regmatches(new.tp.stripped, m)))
        }
    ...
    readAnnotatedTree = function(file = "", text = NULL, tree.names = NULL, 
        skip = 0, comment.char = "#", keep.multi = FALSE, ...) {
        unname = function(treetext) {
            nc = nchar(treetext)
            tstart = 1
            while (substr(treetext, tstart, tstart) != "(" && 
                tstart <= nc) tstart = tstart + 1
            if (tstart > 1) 
        ...
        for (i in 1:Ntree) STRING[i] = paste(tree[x[i]:y[i]], 
            sep = "", collapse = "")
    ...
    nms.annontations.trees = sub("^ *tree *", "", nms.annontations.trees, 
        ignore.case = T)
    ...
    if (any(nms.trees != nms.annontations.trees)) {
        ...
        annotations.trees = lapply(annotations.trees, parse_traits, 
            header = TRUE)
    }
    else {
