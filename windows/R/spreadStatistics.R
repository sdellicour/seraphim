    ...
    outputName = gsub(" ", "_", date()), nberOfCores = 1, slidingWindow = NA) 
{
    ...
    for (t in 1:(nberOfExtractionFiles/N)) {
        ...
        if (onlyTipBranches == TRUE) {
            ...
            for (i in 1:dim(data)[1]) {
                n = data[i, "node2"]
                if (length(data[data[, "node1"] == n, "node1"]) == 
                  0) {
                  indices = c(indices, i)
                }
            }
        ...
        if (t == 1) {
            ...
            if (minLon > min(min(data[, "endLon"]), min(data[, 
                "startLon"]))) 
                minLon = min(min(data[, "endLon"]), min(data[, 
                  "startLon"]))
            if (maxLon < max(max(data[, "endLon"]), max(data[, 
                "startLon"]))) 
                maxLon = max(max(data[, "endLon"]), max(data[, 
                  "startLon"]))
            if (minLat > min(min(data[, "endLat"]), min(data[, 
                "startLat"]))) 
                minLat = min(min(data[, "endLat"]), min(data[, 
                  "startLat"]))
            if (maxLat < max(max(data[, "endLat"]), max(data[, 
                "startLat"]))) 
                maxLat = max(max(data[, "endLat"]), max(data[, 
                  "startLat"]))
            if (minStartYear > min(data[, "startYear"])) 
                minStartYear = min(data[, "startYear"])
            if (minEndYear > min(data[, "endYear"])) 
                minEndYear = min(data[, "endYear"])
    ...
    for (t in 1:(nberOfExtractionFiles/N)) {
        ...
        if (length(grep("greatCircleDist_km", colnames(data))) > 
            0) {
        ...
        weightedDispersalVelocity_numerator = 0
        weightedDispersalVelocity_denominator = 0
        weightedDiffusionCoefficient_numerator = 0
        weightedDiffusionCoefficient_denominator = 0
        for (i in 1:nberOfConnections) {
            dispersalTime = data[i, "endYear"] - data[i, "startYear"]
            branchVelocity = data[i, "greatCircleDist_km"]/(dispersalTime)
            branchOriginalDiffusionCoefficient = (data[i, "greatCircleDist_km"]^2)/(4 * 
                dispersalTime)
            branchMeasures[i, 1] = branchVelocity
            branchMeasures[i, 2] = branchOriginalDiffusionCoefficient
            weightedDispersalVelocity_numerator = weightedDispersalVelocity_numerator + 
                data[i, "greatCircleDist_km"]
            weightedDispersalVelocity_denominator = weightedDispersalVelocity_denominator + 
                dispersalTime
            weightedDiffusionCoefficient_numerator = weightedDiffusionCoefficient_numerator + 
                (data[i, "greatCircleDist_km"]^2)
            weightedDiffusionCoefficient_denominator = weightedDiffusionCoefficient_denominator + 
                (4 * dispersalTime)
        }
        branchVelocities = c(branchVelocities, branchMeasures[, 
            1])
        sd_var_velocity[t, ] = cbind(sd(branchMeasures[, 1]), 
            var(branchMeasures[, 1]))
        ...
        meanStatistics[t, 3] = sd(branchMeasures[, 1])/mean(branchMeasures[, 
            1])
    ...
    if ((onlyTipBranches == FALSE) & (onlyOneAncestor == TRUE)) {
        ...
        for (t in 1:(nberOfExtractionFiles/N)) {
            ...
            waveFrontDistances2 = matrix(nrow = wavefrontDistanceSlices + 
                1, ncol = 2)
            waveFrontDistances1[, 1] = seq(minStartYear, maxEndYear, 
                wavefrontDistanceTimeInterval)
            waveFrontDistances2[, 1] = seq(minStartYear, maxEndYear, 
                wavefrontDistanceTimeInterval)
            data = data[order(data[, "endYear"]), ]
            ancestralIndex = which(data[, "startYear"] == min(data[, 
                "startYear"]))[1]
            ...
            for (i in 0:wavefrontDistanceSlices) {
                ...
                for (j in 1:nberOfConnections) {
                  if ((data[j, "startYear"] < time) & (time < 
                    ...
                    pointLocationX = data[j, "startLon"] + ((data[j, 
                      "endLon"] - data[j, "startLon"]) * timeProportion)
                    pointLocationY = data[j, "startLat"] + ((data[j, 
                      "endLat"] - data[j, "startLat"]) * timeProportion)
                    pointLocation = cbind(pointLocationX, pointLocationY)
                    distance1 = rdist.earth(originLocation, pointLocation, 
                      miles = F, R = NULL)
                    coordinatesOfStartNode = cbind(data[j, "startLon"], 
                      data[j, "startLat"])
    ...
    if ((onlyTipBranches == FALSE) & (dispersalVelocityGraph == 
        ...
        for (i in 1:dispersalVelocitySlices) {
            time = minStartYear + ((i - 1) * dispersalVelocityTimeInterval) + 
                (dispersalVelocityTimeInterval/2)
        ...
        for (t in 1:(nberOfExtractionFiles/N)) {
            ...
            nberOfConnections = dim(data)[1]
            meanDispersalVelocities = matrix(nrow = dispersalVelocitySlices, 
                ncol = 2)
            weightedDispersalVelocities = matrix(nrow = dispersalVelocitySlices, 
                ncol = 2)
            data = data[order(data[, "endYear"]), ]
            for (i in 1:dispersalVelocitySlices) {
                n = 0
                ...
                for (j in 1:nberOfConnections) {
                  ...
                  if ((data[j, "startYear"] < startTime) & (data[j, 
                    "endYear"] > endTime)) 
                    branchInTimeInterval = TRUE
                  if ((data[j, "startYear"] > startTime) & (data[j, 
                    "startYear"] < endTime)) 
                    branchInwavefrontDistanceTimeInterval = TRUE
                  if ((data[j, "endYear"] > startTime) & (data[j, 
                    "endYear"] < endTime)) 
                    branchInwavefrontDistanceTimeInterval = TRUE
                  if (branchInwavefrontDistanceTimeInterval == 
                    TRUE) {
                    if (data[j, "startYear"] > startTime) 
                      time1 = data[j, "startYear"]
                    if (data[j, "startYear"] <= startTime) 
                      time1 = startTime
                    if (data[j, "endYear"] < endTime) 
                      time2 = data[j, "endYear"]
                    if (data[j, "endYear"] >= endTime) 
                      time2 = endTime
                    branchTimeInInterval = time2 - time1
                    timeProportion1 = (time1 - data[j, "startYear"])/(data[j, 
                      "endYear"] - data[j, "startYear"])
                    pointLocation1X = data[j, "startLon"] + ((data[j, 
                      "endLon"] - data[j, "startLon"]) * timeProportion1)
                    pointLocation1Y = data[j, "startLat"] + ((data[j, 
                      "endLat"] - data[j, "startLat"]) * timeProportion1)
                    pointLocation1 = cbind(pointLocation1X, pointLocation1Y)
                    timeProportion2 = (time2 - data[j, "startYear"])/(data[j, 
                      "endYear"] - data[j, "startYear"])
                    pointLocation2X = data[j, "startLon"] + ((data[j, 
                      "endLon"] - data[j, "startLon"]) * timeProportion2)
                    pointLocation2Y = data[j, "startLat"] + ((data[j, 
                      "endLat"] - data[j, "startLat"]) * timeProportion2)
                    pointLocation2 = cbind(pointLocation2X, pointLocation2Y)
                    branchDistInInterval = rdist.earth(pointLocation1, 
                      pointLocation2, miles = F, R = NULL)
                    n = n + 1
                    vS = vS + (branchDistInInterval/branchTimeInInterval)
                    dS = dS + branchDistInInterval
                    tS = tS + branchTimeInInterval
                  }
                }
            ...
            colnames(weightedDispersalVelocities) = c("year", 
                "weightedDispersalVelocity")
    ...
    cat("Median value of mean branch dispersal velocity = ", 
        medianMeanStatistics[1, 1], "\n\t95% HPD = [", ciMeanStatistics[1, 
            1], ", ", ciMeanStatistics[2, 1], "]", "\n", sep = "")
    cat("Median value of weighted branch dispersal velocity = ", 
        medianMeanStatistics[1, 2], "\n\t95% HPD = [", ciMeanStatistics[1, 
            2], ", ", ciMeanStatistics[2, 2], "]", "\n", sep = "")
    cat("Median value of original diffusion coefficient = ", 
        medianMeanStatistics[1, 4], "\n\t95% HPD = [", ciMeanStatistics[1, 
            4], ", ", ciMeanStatistics[2, 4], "]", "\n", sep = "")
    cat("Median value of weighted diffusion coefficient = ", 
        medianMeanStatistics[1, 5], "\n\t95% HPD = [", ciMeanStatistics[1, 
            5], ", ", ciMeanStatistics[2, 5], "]", "\n", sep = "")
    colnames(meanStatistics) = c("mean_branch_dispersal_velocity", 
        "weighted_branch_dispersal_velocity", "branch_dispersal_velocity_variation_among_branches", 
        "original_diffusion_coefficient", "weighted_diffusion_coefficient", 
        "diffusion_coefficient_variation_among_branches")
    ...
    if ((nberOfExtractionFiles/N) > 1) {
        ...
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_mean_branch_dispersal_velocity_variation.pdf", 
                sep = ""), width = 5, height = 5)
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0), lwd = LWD, col = "gray30")
        H = Hpi(cbind(meanStatistics[, 1], meanStatistics[, 3]))
        kde = kde(cbind(meanStatistics[, 1], meanStatistics[, 
            3]), H = H)
        text = "Kernel density estimates of mean branch dispersal velocity parameters"
        colours = c("#FFFFFF", "#D2D3D3", "#9D9FA3", "#6A6A6D")
        xLab = "mean branch velocity"
        yLab = "mean branch velocity variation among branches"
        plot(kde, display = "filled.contour2", cont = c(50, 75, 
            95), col = colours, axes = F, ann = F)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.02, col.axis = "gray30")
        axis(side = 2, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.015, col.axis = "gray30")
        title(xlab = xLab, cex.lab = 0.7, mgp = c(1.4, 0, 0), 
            col.lab = "gray30")
        title(ylab = yLab, cex.lab = 0.7, mgp = c(1.5, 0, 0), 
            col.lab = "gray30")
        title(main = text, cex.main = 0.55, col.main = "gray30")
        legend("bottomright", c("", "95% HPD", "75% HPD", "50% HPD"), 
            text.col = "gray30", pch = 15, pt.cex = 1.5, col = colours, 
            box.lty = 0, cex = 0.6, y.intersp = 1.2)
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_mean_branch_dispersal_velocity_variation.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_weighted_branch_dispersal_velocity_variation.pdf", 
                sep = ""), width = 5, height = 5)
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0), lwd = LWD, col = "gray30")
        H = Hpi(cbind(meanStatistics[, 2], meanStatistics[, 3]))
        kde = kde(cbind(meanStatistics[, 2], meanStatistics[, 
            3]), H = H)
        text = "Kernel density estimates of weighted branch dispersal velocity parameters"
        colours = c("#FFFFFF", "#D2D3D3", "#9D9FA3", "#6A6A6D")
        xLab = "weighted dispersal velocity"
        yLab = "weighted dispersal velocity variation among branches"
        plot(kde, display = "filled.contour2", cont = c(50, 75, 
            95), col = colours, axes = F, ann = F)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.02, col.axis = "gray30")
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_weighted_branch_dispersal_velocity_variation.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_original_diffusion_coefficient_variation.pdf", 
                sep = ""), width = 5, height = 5)
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0), lwd = LWD, col = "gray30")
        H = Hpi(cbind(meanStatistics[, 4], meanStatistics[, 6]))
        kde = kde(cbind(meanStatistics[, 4], meanStatistics[, 
            6]), H = H)
        text1 = "Kernel density estimates of original diffusion coefficient parameters"
        text2 = "(Pybus et al. 2012)"
        colours = c("#FFFFFF", "#D2D3D3", "#9D9FA3", "#6A6A6D")
        xLab = "mean original diffusion coefficient"
        yLab = "diffusion coefficient variation among branches"
        plot(kde, display = "filled.contour2", cont = c(50, 75, 
            95), col = colours, axes = F, ann = F)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.02, col.axis = "gray30")
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_original_diffusion_coefficient_variation.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_weighted_diffusion_coefficient_variation.pdf", 
                sep = ""), width = 5, height = 5)
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0), lwd = LWD, col = "gray30")
        H = Hpi(cbind(meanStatistics[, 5], meanStatistics[, 6]))
        kde = kde(cbind(meanStatistics[, 5], meanStatistics[, 
            6]), H = H)
        text1 = "Kernel density estimates of weighted diffusion coefficient parameters"
        text2 = "(Trovao et al. 2015)"
        colours = c("#FFFFFF", "#D2D3D3", "#9D9FA3", "#6A6A6D")
        xLab = "mean weighted diffusion coefficient"
        yLab = "diffusion coefficient variation among branches"
        plot(kde, display = "filled.contour2", cont = c(50, 75, 
            95), col = colours, axes = F, ann = F)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_weighted_diffusion_coefficient_variation.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
    }
    if ((onlyTipBranches == FALSE) & (onlyOneAncestor == TRUE)) {
        cat("Building wavefront distance evolution graphs", "\n", 
            sep = "")
        wavefrontDistanceTimeInterval = (maxEndYear - minStartYear)/wavefrontDistanceSlices
        slicedTimes = matrix(nrow = 1, ncol = (wavefrontDistanceSlices + 
            1))
        lower_l_1 = matrix(nrow = 1, ncol = (wavefrontDistanceSlices + 
            1))
        upper_l_1 = matrix(nrow = 1, ncol = (wavefrontDistanceSlices + 
            1))
        lower_l_2 = matrix(nrow = 1, ncol = (wavefrontDistanceSlices + 
            1))
        ...
        for (i in 0:wavefrontDistanceSlices) {
            ...
            quantiles = quantile(waveFrontDistances2Values[, 
                i], probs = c(0.025, 0.975))
            lower_l_2[1, i + 1] = as.numeric(quantiles[1])
            upper_l_2[1, i + 1] = as.numeric(quantiles[2])
            waveFrontDistances1MeanValue[1, i + 1] = mean(waveFrontDistances1Values[, 
                i + 1])
            waveFrontDistances1MedianValue[1, i + 1] = median(waveFrontDistances1Values[, 
                i + 1])
            waveFrontDistances2MeanValue[1, i + 1] = mean(waveFrontDistances2Values[, 
                i + 1])
            waveFrontDistances2MedianValue[1, i + 1] = median(waveFrontDistances2Values[, 
                i + 1])
        }
        ...
        tab[, 1] = slicedTimes
        tab[, 2] = waveFrontDistances1MeanValue
        colnames(tab) = c("time", "distance")
        write.table(tab, file = paste(outputName, "_mean_spatial_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = waveFrontDistances1MedianValue
        colnames(tab) = c("time", "distance")
        write.table(tab, file = paste(outputName, "_median_spatial_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = waveFrontDistances2MeanValue
        colnames(tab) = c("time", "distance")
        write.table(tab, file = paste(outputName, "_mean_patristic_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = waveFrontDistances2MedianValue
        colnames(tab) = c("time", "distance")
        write.table(tab, file = paste(outputName, "_median_patristic_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab = matrix(nrow = length(slicedTimes), ncol = 3)
        tab[, 1] = slicedTimes
        tab[, 2] = lower_l_1
        tab[, 3] = upper_l_1
        colnames(tab) = c("time", "95%HPD_lower_value", "95%HPD_higher_value")
        write.table(tab, file = paste(outputName, "_95%HPD_spatial_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = lower_l_2
        tab[, 3] = upper_l_2
        colnames(tab) = c("time", "95%HPD_lower_value", "95%HPD_higher_value")
        write.table(tab, file = paste(outputName, "_95%HPD_patristic_wavefront_distance.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        ...
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_spatial_wavefront_distance_1.pdf", 
                sep = ""), width = 5, height = 5)
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_spatial_wavefront_distance_1.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_patristic_wavefront_distance_1.pdf", 
                sep = ""), width = 5, height = 5)
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_patristic_wavefront_distance_1.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_spatial_wavefront_distance_2.pdf", 
                sep = ""), width = 5, height = 5)
        text = "Furthest extent of epidemic wavefront (spatial distance from epidemic origin)"
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0))
        plot(slicedTimes, waveFrontDistances1MedianValue, type = "l", 
            axes = F, ann = F, ylim = yLim1, xlim = xLim)
        xx_l = c(slicedTimes, rev(slicedTimes))
        yy_l = c(lower_l_1, rev(upper_l_1))
        getOption("scipen")
        opt = options(scipen = 20)
        polygon(xx_l, yy_l, col = rgb(187/255, 187/255, 187/255, 
            0.5), border = 0)
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_spatial_wavefront_distance_2.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_patristic_wavefront_distance_2.pdf", 
                sep = ""), width = 5, height = 5)
        text = "Evolution of epidemic wavefront (patristic distance from epidemic origin)"
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0))
        plot(slicedTimes, waveFrontDistances2MedianValue, type = "l", 
            axes = F, ann = F, ylim = yLim2, xlim = xLim)
        xx_l = c(slicedTimes, rev(slicedTimes))
        yy_l = c(lower_l_2, rev(upper_l_2))
        getOption("scipen")
        opt = options(scipen = 20)
        polygon(xx_l, yy_l, col = rgb(187/255, 187/255, 187/255, 
            0.5), border = 0)
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_patristic_wavefront_distance_2.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
    }
    if ((onlyTipBranches == FALSE) & (dispersalVelocityGraph == 
        ...
        for (i in 1:dispersalVelocitySlices) {
            ...
            quantiles = quantile(meanDispersalVelocitiesValues[, 
                i], probs = c(0.025, 0.975), na.rm = T)
            lower_l[1, i] = as.numeric(quantiles[1])
            upper_l[1, i] = as.numeric(quantiles[2])
            meanDispersalVelocitiesMeanValue[1, i] = mean(meanDispersalVelocitiesValues[, 
                i], na.rm = T)
            meanDispersalVelocitiesMedianValue[1, i] = median(meanDispersalVelocitiesValues[, 
                i], na.rm = T)
        }
        ...
        tab = matrix(nrow = length(slicedTimes), ncol = 2)
        tab[, 1] = slicedTimes
        tab[, 2] = meanDispersalVelocitiesMeanValue
        colnames(tab) = c("time", "velocity")
        write.table(tab, file = paste(outputName, "_mean_mean_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = meanDispersalVelocitiesMedianValue
        colnames(tab) = c("time", "velocity")
        write.table(tab, file = paste(outputName, "_median_mean_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab = matrix(nrow = length(slicedTimes), ncol = 3)
        tab[, 1] = slicedTimes
        tab[, 2] = lower_l
        tab[, 3] = upper_l
        colnames(tab) = c("time", "95%HPD_lower_value", "95%HPD_higher_value")
        write.table(tab, file = paste(outputName, "_95%HPD_mean_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        xLab = "time"
        yLab = "weighted branch dispersal velocity"
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_mean_branch_dispersal_velocity_1.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
        ...
        plot(slicedTimes, meanDispersalVelocitiesMedianValue, 
            type = "l", axes = F, ann = F, ylim = yLim, xlim = xLim)
        xx_l = c(slicedTimes, rev(slicedTimes))
        yy_l = c(lower_l, rev(upper_l))
        getOption("scipen")
        opt = options(scipen = 20)
        polygon(xx_l, yy_l, col = rgb(187/255, 187/255, 187/255, 
            0.5), border = 0)
        lines(slicedTimes, meanDispersalVelocitiesMedianValue, 
            lwd = 1)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.02, col.axis = "gray30")
        axis(side = 2, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_mean_branch_dispersal_velocity_2.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        slicedTimes = matrix(nrow = 1, ncol = dispersalVelocitySlices)
        ...
        for (i in 1:dispersalVelocitySlices) {
            ...
            quantiles = quantile(weightedDispersalVelocitiesValues[, 
                i], probs = c(0.025, 0.975), na.rm = T)
            lower_l[1, i] = as.numeric(quantiles[1])
            upper_l[1, i] = as.numeric(quantiles[2])
            weightedDispersalVelocitiesMeanValue[1, i] = mean(weightedDispersalVelocitiesValues[, 
                i], na.rm = T)
            weightedDispersalVelocitiesMedianValue[1, i] = median(weightedDispersalVelocitiesValues[, 
                i], na.rm = T)
        }
        yLim = c(0, max(upper_l, na.rm = T))
        treeIDs = paste("distance_tree", treeIDs, sep = "")
        tab = matrix(nrow = length(slicedTimes), ncol = 2)
        tab[, 1] = slicedTimes
        tab[, 2] = weightedDispersalVelocitiesMeanValue
        colnames(tab) = c("time", "velocity")
        write.table(tab, file = paste(outputName, "_mean_weighted_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab[, 1] = slicedTimes
        tab[, 2] = weightedDispersalVelocitiesMedianValue
        colnames(tab) = c("time", "velocity")
        write.table(tab, file = paste(outputName, "_median_weighted_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        tab = matrix(nrow = length(slicedTimes), ncol = 3)
        tab[, 1] = slicedTimes
        tab[, 2] = lower_l
        tab[, 3] = upper_l
        colnames(tab) = c("time", "95%HPD_lower_value", "95%HPD_higher_value")
        write.table(tab, file = paste(outputName, "_95%HPD_weighted_branch_dispersal_velocity.txt", 
            sep = ""), row.names = F, quote = F, sep = "\t")
        xLab = "time"
        yLab = "weighted branch dispersal velocity"
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_weighted_branch_dispersal_velocity_1.pdf", 
                sep = ""), width = 5, height = 5)
        text = "Evolution of weighted branch dispersal velocity"
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0))
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_weighted_branch_dispersal_velocity_1.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
        if (showingPlots) 
            dev.new(width = 5, height = 5)
        if (showingPlots == FALSE) 
            pdf(paste(outputName, "_weighted_branch_dispersal_velocity_2.pdf", 
                sep = ""), width = 5, height = 5)
        text = "Evolution of weighted branch dispersal velocity"
        par(mgp = c(1, 0.35, 0), oma = c(1, 1, 1.5, 3), mar = c(3.3, 
            3.3, 2, 0))
        plot(slicedTimes, weightedDispersalVelocitiesMedianValue, 
            type = "l", axes = F, ann = F, ylim = yLim, xlim = xLim)
        xx_l = c(slicedTimes, rev(slicedTimes))
        yy_l = c(lower_l, rev(upper_l))
        getOption("scipen")
        ...
        polygon(xx_l, yy_l, col = rgb(187/255, 187/255, 187/255, 
            0.5), border = 0)
        lines(slicedTimes, weightedDispersalVelocitiesMedianValue, 
            lwd = 1)
        axis(side = 1, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
            tck = -0.02, col.axis = "gray30")
        axis(side = 2, lwd.tick = LWD, cex.axis = 0.6, lwd = 0, 
        ...
        box(lwd = LWD, col = "gray30")
        if (showingPlots) 
            dev.copy2pdf(file = paste(outputName, "_weighted_branch_dispersal_velocity_2.pdf", 
                sep = ""))
        if (showingPlots == FALSE) 
            dev.off()
    }
