    ...
    n1 = 100, n2 = 0, showingPlots = FALSE, newPlot = TRUE, fixingRootBranches = FALSE) 
{
    ...
    if (showingPlots == TRUE) {
        if ((newPlot == TRUE) & (length(envVariables) > 0)) {
            plotRaster(rast = envVariables[[1]], cols = "gray90", 
                colNA = "white", addBox = F)
        }
        points(cbind(ancestPosition[1], ancestPosition[2]), pch = 16, 
            col = pointCol, cex = 0.5)
    }
    i = 0
    while (i != (nd_max - 1)) {
        i = i + 1
        ...
        if (length(my_nodes) > 0) {
            for (j in 1:length(my_nodes)) {
                ...
                if (simulatingNode == TRUE) {
                  ...
                  increment2 = sigmas[2] * ((cor * increment1) + 
                    (sqrt(1 - cor^2) * increment2))
            ...
            if (i == 0) {
                ...
                if (showingPlots == TRUE) {
                  plotRaster(rast = envVariables[[1]], cols = "gray90", 
                    colNA = "white", addBox = F, new = F)
                  points(cbind(ancestPosition[1], ancestPosition[2]), 
                    pch = 16, col = pointCol, cex = 0.5)
                }
            }
        }
    }
    ...
    for (i in 1:dim(tree$edge)[1]) {
        ...
        x1 = cbind(x[node_i], y[node_i])
        x2 = cbind(x[node_f], y[node_f])
        simulation[i, "greatCircleDist_km"] = rdist.earth(x1, 
            x2, miles = FALSE, R = NULL)
    }
    ...
    for (j in 1:l) {
        ...
        for (k in 1:l) {
            if (nrow(subMat1) > 0) {
                ll[j, k + 1] = subMat1[, 3]
                subMat2 = subMat1[1, 1]
                subMat1 = subset(simulation, simulation[, 2] == 
                  subMat2)
            }
        }
    }
    ...
    startYear = matrix(mostRecentSamplingDatum + (startYear[, 
        1]))
    simulation[, c("startYear", "endYear")] = cbind(startYear, 
        endYear)
    if (showingPlots == TRUE) 
        dev.off()
    return(simulation)
