...
function (localTreesDirectory = "", nberOfExtractionFiles = 1, 
    envVariables = list(), randomProcedure = 3, nberOfCores = 1, 
    showingPlots = F) 
{
        ...
        for (s in 1:nberOfRandomisations) {
            if (branchRandomisation3 == TRUE) {
                ...
                buffer = foreach(t = 1:nberOfExtractionFiles) %dopar% 
                  {
                    if (!file.exists(paste0(localTreesDirectory, 
                      "/TreeRandomisation_", t, ".csv"))) {
                      counter1 = 0
                      ...
                      temp = datas[[t]]
                      temp[, c("startLon", "startLat")] = fromCoorRand[[t]]
                      temp[, c("endLon", "endLat")] = toCoorRand[[t]]
                      write.csv(temp, paste0(localTreesDirectory, 
                        "/TreeRandomisation_", t, ".csv"), row.names = F, 
                        quote = F)
                    }
                    t
                  }
                ...
                buffer = foreach(t = 1:nberOfExtractionFiles) %dopar% 
                  {
                    if (!file.exists(paste0(localTreesDirectory, 
                      ...
                      temp[, c("startLon", "startLat")] = fromCoorRand[[t]]
                      temp[, c("endLon", "endLat")] = toCoorRand[[t]]
                      write.csv(temp, paste0(localTreeDirectory, 
                        "/TreeRandomisation_", t, ".csv"), row.names = F, 
                        quote = F)
                    }
                    t
                      ...
                      for (i in 1:nberOfConnections[t]) {
                        ...
                        while (twoPointsOnTheGrid == FALSE) {
                          ...
                          while (pt1NAarea == TRUE) {
                            ...
                            if (insideAtLeastOneHullPolygon == 
                              FALSE) 
                              NAarea = TRUE
                            if (NAarea == FALSE) {
                              pt1NAarea = FALSE
