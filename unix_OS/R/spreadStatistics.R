        ...
        buffer = foreach(t = 1:nberOfExtractionFiles) %dopar% 
            {
                ...
                for (i in 1:dispersalVelocitySlices) {
                  ...
                  for (j in 1:nberOfConnections) {
                    ...
                    if ((data[j, "startYear"] < startTime) & 
                      (data[j, "endYear"] > endTime)) 
                      branchInTimeInterval = TRUE
                    if ((data[j, "startYear"] > startTime) & 
                      (data[j, "startYear"] < endTime)) 
                    ...
                    if (branchInwavefrontDistanceTimeInterval == 
                      ...
                      timeProportion1 = (time1 - data[j, "startYear"])/(data[j, 
                        "endYear"] - data[j, "startYear"])
                      pointLocation1X = data[j, "startLon"] + 
                        ((data[j, "endLon"] - data[j, "startLon"]) * 
                          timeProportion1)
                      pointLocation1Y = data[j, "startLat"] + 
                        ((data[j, "endLat"] - data[j, "startLat"]) * 
                          timeProportion1)
                      pointLocation1 = cbind(pointLocation1X, 
                        pointLocation1Y)
                      timeProportion2 = (time2 - data[j, "startYear"])/(data[j, 
                        "endYear"] - data[j, "startYear"])
                      pointLocation2X = data[j, "startLon"] + 
                        ((data[j, "endLon"] - data[j, "startLon"]) * 
                          timeProportion2)
                      pointLocation2Y = data[j, "startLat"] + 
                        ((data[j, "endLat"] - data[j, "startLat"]) * 
                          timeProportion2)
                      pointLocation2 = cbind(pointLocation2X, 
                        pointLocation2Y)
                ...
                colnames(meanDispersalVelocities) = c("year", 
                  "meanDispersalVelocity")
