...
function (envVariables) 
{
    ...
    for (i in 1:length(envVariables)) {
        ...
        samples = as.data.frame(sampleRandom(rast, nberOfPoints, 
            sp = T))
        coordinates(samples) = ~x + y
        fmla = as.formula(paste(rast@data@names, "~1", sep = ""))
        observedVariogram = variogram(fmla, data = samples)
        dev.new(width = 5, height = 5)
        par(mar = c(3.5, 3.5, 4.5, 2))
        par(mgp = c(1.6, 0.5, 0))
        text1 = paste("Observed variogram for ", rast@data@names, 
            "*", sep = "")
        text2 = paste("User-defined variogram for ", rast@data@names, 
            "*", sep = "")
        text3 = paste("Fitted (corrected) variogram for ", rast@data@names, 
            "*", sep = "")
        text4 = paste("* a normal transformation was                 ", 
            sep = "")
        text5 = paste(" previously performed on the raster grid   ", 
            sep = "")
        plot(observedVariogram$dist, observedVariogram$gamma, 
            xlab = "distance", ylab = "semivariance", cex.lab = 0.7, 
            cex.axis = 0.7)
        ...
        while (happyWithModel == FALSE) {
            cat("Setting parameters value for variogram model fitting (", 
                rast@data@names, "):", sep = "")
            ...
            fittedVariogramModel = fit.variogram(observedVariogram, 
                userVariogramModel)
            mtext(text2, cex = 0.8, line = 1.6, col = "blue")
            lines(variogramLine(userVariogramModel, max(observedVariogram$dist)), 
                col = "blue", lwd = 2, lty = 2)
            Sys.sleep(2)
            mtext(text3, cex = 0.8, line = 0.7, col = "red")
            lines(variogramLine(fittedVariogramModel, max(observedVariogram$dist)), 
                col = "red", lwd = 2)
        ...
        dev.off()
        variogramModel = gstat(formula = fmla, dummy = T, beta = 0, 
            model = fittedVariogramModel, nmax = round(fittedVariogramModel$range[2] * 
                10))
        variogramModels[[i]] = variogramModel
        nScoreTransformation = FALSE
        quantileTransformation = FALSE
        if (nScoreTransformation == TRUE) {
            rastScore = nscore(rast[])
            simRast = rast
            simRast[] = rastScore$nscore
            mask = simRast * 0 + 1
            mask.grid = as(mask, "SpatialGridDataFrame")
            simulation = predict(variogramModels[[h]], newdata = mask.grid, 
                nsim = 1)
        ...
        if (nScoreTransformation == TRUE) {
            ...
            simRast[] = simRastData[, "value"]
            rangeSimRast = max(simRast[], na.rm = T) - min(simRast[], 
                na.rm = T)
            rangeRast = max(rast[], na.rm = T) - min(rast[], 
                na.rm = T)
